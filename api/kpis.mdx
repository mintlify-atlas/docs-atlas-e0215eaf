...
import type { KpisParams, KpisOutput } from './endpoints';

const tb = new Tinybird({ token: process.env.TINYBIRD_TOKEN });

// Get KPIs for the last 7 days (default)
const result = await tb.exec<KpisOutput>('kpis');
result.data.forEach(day => {
  console.log(`${day.date}: ${day.visits} visits, ${day.pageviews} pageviews`);
});

// Get KPIs for a specific date range
const params: KpisParams = {
  date_from: '2024-01-01',
  date_to: '2024-01-31',
  domain: 'example.com'
};
const rangeResult = await tb.exec<KpisOutput>('kpis', params);

// Get KPIs with previous period comparison
const comparisonParams: KpisParams = {
  date_from: '2024-01-01',
  date_to: '2024-01-07',
  include_previous_period: 'true'
};
const comparisonResult = await tb.exec<KpisOutput>('kpis', comparisonParams);
