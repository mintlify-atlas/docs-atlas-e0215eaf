...
import type { TrendParams, TrendOutput } from './endpoints';

const tb = new Tinybird({ token: process.env.TINYBIRD_TOKEN });

// Get trend for all domains
const result = await tb.exec<TrendOutput>('trend');
...
const domainResult = await tb.exec<TrendOutput>('trend', params);
...
const tenantParams: TrendParams = {
  tenant_id: 'tenant-123',
  domain: 'example.com'
};
const tenantResult = await tb.exec<TrendOutput>('trend', tenantParams);
```
